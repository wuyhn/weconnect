package com.example.weconnect.presentation.post.detail;
...
public class PostDetailActivity extends AppCompatActivity {
        ...
        postApi.approveMember(postId, userId).enqueue(new Callback<ApiResponse<Void>>() {
            ...
            public void onResponse(Call<ApiResponse<Void>> call, Response<ApiResponse<Void>> response) {
                if (response.isSuccessful()) {
                    ...
                    // Create/update activity group chat and add the approved member
                    String chatTitle = post.getInterestTag() != null && !post.getInterestTag().isEmpty()
                            ? post.getInterestTag() : post.getContent();
                    com.example.weconnect.data.repository.FakeChatRepository chatRepo =
                            com.example.weconnect.data.repository.FakeChatRepository.getInstance();
                    chatRepo.getOrCreateActivityGroupChat(post.getId(), chatTitle, post.getUsername());
                    if (userName != null) {
                        chatRepo.addMemberToActivityChat(post.getId(), userName);
                    }
    ...
    private void openRelatedGroupChat() {
        ...
        // Get or create the activity group chat room
        String chatTitle = post.getInterestTag() != null && !post.getInterestTag().isEmpty()
                ? post.getInterestTag() : post.getContent();
        com.example.weconnect.data.repository.FakeChatRepository chatRepo =
                com.example.weconnect.data.repository.FakeChatRepository.getInstance();
        com.example.weconnect.data.model.ChatRoom room =
                chatRepo.getOrCreateActivityGroupChat(post.getId(), chatTitle, post.getUsername());
